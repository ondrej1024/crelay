...
/* Definition of reserved characters */
#define COMMENT_CHAR '#'
...
/* Return pointer to first char c or COMMENT_CHAR comment in given string, or pointer to
   null at end of string if neither found. COMMENT_CHAR must be prefixed by a whitespace
   character to register as a comment. */
static char* find_char_or_comment(const char* s, char c)
{
    int was_whitespace = 0;
    while (*s && *s != c && !(was_whitespace && *s == COMMENT_CHAR)) {
