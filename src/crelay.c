...
static int config_cb(void* user, const char* section, const char* name, const char* value)
{
   ...
   else if (MATCH("MQTT broker", "mqtt_host"))
   {
      pconfig->mqtt_host = strdup(value);
   }
   else if (MATCH("MQTT broker", "mqtt_port"))
   {
      pconfig->mqtt_port = atoi(value);
   }
   else if (MATCH("MQTT broker", "crelay_custom_id"))
   {
      pconfig->crelay_custom_id = strdup(value);
   }
...
int main(int argc, char *argv[])
{
   ...
   if (!strcmp(argv[1],"-d") || !strcmp(argv[1],"-D"))
   {
      ...
      if (conf_parse(CONFIG_FILE, config_cb, &config) >= 0)
      {
         ...
         if (config.mqtt_host != NULL)    syslog(LOG_DAEMON | LOG_NOTICE, "mqtt_host: %s\n", config.mqtt_host);
         if (config.mqtt_port != 0)       syslog(LOG_DAEMON | LOG_NOTICE, "mqtt_port: %u\n", config.mqtt_port);
         if (config.crelay_custom_id != NULL) syslog(LOG_DAEMON | LOG_NOTICE, "crelay_custom_id: %s\n", config.crelay_custom_id);
